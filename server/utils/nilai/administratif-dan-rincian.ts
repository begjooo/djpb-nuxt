...
export function hitungPengumpulan(triwulan: string, waktuPengumpulan: string){
  ...
  const hasilAnalisis = {
    batas: triwulan,
    pengumpulan: waktuPengumpulan,
    alasan: `${selisihHari} hari setelah batas TW`,
    nilai: nilai,
    nilaiAkhir: nilai * 0.2 * 0.4,
  };
...
export async function hitungNilaiAdministratif(file: DriveFile): Promise<any> {
  ...
  if(file.fields['NilaiAdministratif'] === undefined || file.fields['NilaiAdministratif'] === ''){
    hasilAnalisis = await analisisPDF(file);
    // hasilAnalisis = {
    //   "format": {
    //     "Kata Pengantar": true,
    ...
    // console.log('nilaiFormat:', nilaiFormat);
    hasilAnalisis.format.nilai = nilaiFormat.nilai;
    hasilAnalisis.format.alasan = nilaiFormat.keterangan;
    // console.log(hasilAnalisis);
