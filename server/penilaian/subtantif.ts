...
async function analisisPihakKegiatan(daftarKegiatanJson: any[]): Promise<any> {
  ...
  promptMap.set('kegiatanPihak', kegiatanPihakJsonPrompt2);
  let kegiatanPihakJson: any = null;
  try {
    const genAiResponse = await processTextWithGemini(
      daftarKegiatanPihakString,
      promptMap.get('kegiatanPihak'),
      kegiatanPihakJsonSchema2,
    );
...
function hitungKegiatanNonDjpb2(daftarKegiatanJson: any[], totalKegiatan: number){
...
export async function hitungNilaiSubstantif(file: DriveFile): Promise<any> {
  ...
  if(file.fields['NilaiSubstantif'] === undefined || file.fields['NilaiSubstantif'] === ''){
    ...
    const nilaiKegiatanNonDjpb = hitungKegiatanNonDjpb2(pihakKegiatan, nKegiatan);
